  ...
  function createBoardPage(){
    ...
    for (let i = firstPageInBoard; i < sc.totalPage + 1; i++) {//시작페이지부터 총페이지수까지
      if (sc.currentBlock === 1) {//case1 : 1페이지일경우
        html += "<li class=\"btn btn-icon btn-light-primary mr-2 my-1\" onclick='getBoardsByPageNum("+i+",orderRule)'>"+i+"</li>";//[1]~[5]찍어주구
        ...
        html += "<li class=\"btn btn-icon btn-light-primary mr-2 my-1\" onclick='getBoardsByPageNum("+i+",orderRule)'>"+i+"</li>";
        /*html += "<li><a href='/study/challenge/list?page=" + i + "'>" + i + "</a></li>";*/
        //[6]~[10] 찍어주고 끝내겠다
    ...
    if (sc.currentBlock != sc.totalBlock) {
      html += "<li class=\"btn btn-icon btn-light-primary mr-2 my-1\" onclick='getBoardsByPageNum("+sc.totalPage+",orderRule)'>"+ sc.totalPage + "</li>";
